...
export function createRun(seed,hero='kaerun',options={}){
 ...
 if(!options.legacy){
  ...
  else{const beastRouteState={rng:hash(seed+':beast-route')};for(let row=0;row<layouts.length-1;row++){if(random(beastRouteState)<.4){let choices=layouts[row].map((type,col)=>({type,col})).filter(x=>x.type==='battle');if(!choices.length)choices=layouts[row].map((type,col)=>({type,col})).filter(x=>x.type==='mystery');if(!choices.length)choices=layouts[row].map((type,col)=>({type,col}));const pick=choices[Math.floor(random(beastRouteState)*choices.length)];layouts[row][pick.col]='beast';}}}
 }
